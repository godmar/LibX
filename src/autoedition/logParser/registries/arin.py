...
"""
    ...
    try:
        ...
        result = {}
        result['start'] = convertIP(ips[0])
        result['end'] = convertIP(ips[1])
        result['cidrs'] = []
        ...
        for current_ip in ipList:
            ...
            result['cidrs'].append((newStart, newValue))
        #for current_ip, current_value in result[2:]:
        #    print(deconvertIP(current_ip) + "/" + str(current_value))
    except:
        print("Error in the ARIN callback: " + sys.exc_info()[0])
        result = None
    finally:
        print("Calling back from ARIN")
