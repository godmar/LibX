...
// LibX: This file has been changed so that each ul must have the "checktree" class
// in order to be a checktree. This allows us to have a tree mixed with nodes that
// do and do not have checkboxes.
...
jQuery.fn.checkTree = function(settings) {
    ...
    $tree.find("li")
        ...
        .each(function() {
            ...
            jQuery(this).find("ul.tree").each(function() {
                if (!jQuery(this).siblings(".expanded").length) jQuery(this).hide();
            });
            jQuery(this).find("ul.tree > li > ul").each(function() {
                jQuery(this).hide();
            });
